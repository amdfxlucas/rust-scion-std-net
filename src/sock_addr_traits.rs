...
use std::net::ToSocketAddrs as netToSocketAddrs;
use crate::ScionAddr;
...
use libc as c;
//use std::sys_common::net::LookupHost;
//use std::sys_common::{FromInner, IntoInner};
use std::vec;


pub use crate::{SocketAddr, SocketAddrV4, SocketAddrV6};
...
impl ToSocketAddrs for (&str, u16) {
    ...
    fn to_socket_addrs(&self) -> io::Result<vec::IntoIter<SocketAddr>> {
        ...
        if let Ok(addr) = host.parse::<ScionAddr>() {
            let addr = SocketAddrScion::new1( addr,port);
            return Ok(vec![SocketAddr::SCION(addr)].into_iter())
        }

       // TODO use pan resolve_udp_addr() here to obtain scion address for domain names
       let x = netToSocketAddrs::to_socket_addrs(self)
       .map(|x|{x.map(Into::<SocketAddr>::into).collect::<Vec<_>>().into_iter() } ) ;// ::<(&str,u16)>
        x
...
impl ToSocketAddrs for (String, u16) {
    ...
    fn to_socket_addrs(&self) -> io::Result<vec::IntoIter<SocketAddr>> {
      ...
      self::ToSocketAddrs::to_socket_addrs(&(&*self.0, self.1))
    }
}
...
impl ToSocketAddrs for str {
    ...
    fn to_socket_addrs(&self) -> io::Result<vec::IntoIter<SocketAddr>> {
        ...
        // TODO use pan audp_addr() here to obtain scion address for domain names
       // resolve_socket_addr(self.try_into()?)
       std::net::ToSocketAddrs::to_socket_addrs(self)
       .map(|x|{x.map(Into::<SocketAddr>::into).collect::<Vec<_>>().into_iter() } ) 
    }
}
...
impl ToSocketAddrs for String {
    ...
    fn to_socket_addrs(&self) -> io::Result<vec::IntoIter<SocketAddr>> {
       // (&**self).to_socket_addrs()
       self::ToSocketAddrs::to_socket_addrs(&(&**self))
    }
}